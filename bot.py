...
def joy_response(language):
    embed = discord.Embed(
        ...
        description= google_translate("""Happiness looks different for everyone, but it is often described as involving positive emotions and life satisfaction. Perhaps you've had a rough few days and things have just started to look up. Everyone experiences the ups and downs of life, but today is one of the good ones 😊.""", language)["translatedText"], 
        timestamp=datetime.datetime.utcnow(), 
        color=discord.Color.from_rgb(252, 252, 153)
    )
    embed.add_field(name=google_translate("Advice", language)["translatedText"], value=google_translate("Take the time to do something you enjoy, develop positive habits or work towards one of your goals today to increase your happiness and life satisfaction in the long run.""" +
                    """\nGive this article a read for tips to maintain happiness: """, language)["translatedText"])
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name=google_translate("Resource", language)["translatedText"], value=f"[{google_translate('10 Things Happy People Do to Stay Happy', language)['translatedText']}](https://tinybuddha.com/blog/10-things-happy-people-do-to-stay-happy/)")
...
def sad_response(language):
    embed = discord.Embed(
        ...
        description=google_translate("We all feel sad at times. Sadness is a normal human emotion that can make life more interesting and teach us to appreciate happiness when it comes. That said, sadness can be overwhelming at times and lead us into a downward spiral if not dealt with. Here are some tips to work through sadness:", language)["translatedText"],
        timestamp=datetime.datetime.utcnow(), 
        color=discord.Color.from_rgb(168, 228, 239)
    )
    
    embed.add_field(name=google_translate("Get out in nature", language)["translatedText"], value=google_translate(" Spending time outdoors can improve your mood and take your mind off your problems and widen your perspectives to beyond the current situation.", language)["translatedText"])
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name='\u200b', value='\u200b')

    embed.add_field(name=google_translate("Talk to someone.", language)["translatedText"], value=google_translate("This could be a friend, therapist, or family member, someone you trust to listen to and comfort you if needed. Many online services such as KidsHelpPhone also provide support for those in need.", language)["translatedText"])
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name='\u200b', value='\u200b')

    embed.add_field(name=google_translate("Take it one day at a time", language)["translatedText"], value=google_translate("Do one thing to work towards improving your mental state today, such as going for a walk or connecting with an old friend. Do another tomorrow. Over time these little actions will build up.", language)["translatedText"])
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name='\u200b', value='\u200b')

    embed.add_field(name=google_translate("Give yourself time and permission to be sad", language)["translatedText"], value=google_translate("Bottling up your feelings will do more harm than good over time. Don't be afraid to seek help or express your feelings through crying if you feel like it. ", language)["translatedText"])
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name='\u200b', value='\u200b')
    
    embed.add_field(name=google_translate("Remember, it will get better", language)["translatedText"], value=google_translate(" You may not feel like it now, but what may seem so significant today may not even matter in the future.", language)["translatedText"])
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name='\u200b', value='\u200b')
    
    embed.add_field(name=google_translate("Resource", language)["translatedText"], value=f"[{google_translate('10 Ways to Cheer Yourself Up When You’re in a Bad Mood', language)['translatedText']}](https://www.lifehack.org/articles/lifestyle/10-ways-cheer-yourself-when-youre-bad-mood.html)")
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/845318994666848261/845399136249053205/logo_guy.png")
    return embed
...
def anger_response(language):
    embed = discord.Embed(
        ...
        description=google_translate("""Remember that it’s perfectly reasonable to get angry. Something that can help is letting out your anger and venting;""" + 
                    """ holding it in can make it worse. Try out the resource below, or use `>resource` for some more great resources to help improve your mental health.""", language)["translatedText"],
        timestamp=datetime.datetime.utcnow(), 
        color=discord.Color.from_rgb(251, 105, 98)
    )
    embed.add_field(name=google_translate("Resource", language)["translatedText"], value=f"[{google_translate('How To Cool Off When Youre Angry', language)['translatedText']}](https://www.thehotline.org/resources/how-to-cool-off-when-youre-angry/)")
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/845318994666848261/845399136249053205/logo_guy.png")
    return embed
...
def fear_response(language):
    embed = discord.Embed(
        ...
        description= google_translate("""I totally get that life can be overwhelming and stressful. You might be able to get over your fears by trying """ + 
                    """to take a step back and distracting yourself. You can often get in your own head and make your fears worse. """ + 
                    """Try the resource below, or use `>resource` for some more great resources to help improve your mental health.""", language)["translatedText"],
        timestamp=datetime.datetime.utcnow(), 
        color=discord.Color.from_rgb(168, 228, 239)
    )
    embed.add_field(name=google_translate("Resource", language)["translatedText"], value=f"[**{google_translate('NHS Inform', language)['translatedText']}** - {google_translate('Ten Ways to Fight Your Fears', language)['translatedText']}](https://www.nhsinform.scot/healthy-living/mental-wellbeing/fears-and-phobias/ten-ways-to-fight-your-fears)")
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/845318994666848261/845399136249053205/logo_guy.png")
    return embed
   
def no_tone(language):
    embed = discord.Embed(title=google_translate("Oops!", language)['translatedText'], description=google_translate("Sorry! I couldn't quite pinpoint how you are feeling. Feel free to send me another message and I'll try to figure out how you are feeling!", language)["translatedText"], timestamp=datetime.datetime.utcnow(), color=discord.Color.from_rgb(61, 72, 73))
    embed.add_field(name=">rate", value=google_translate("You can also let me know how you are feeling with `>rate`!", language)['translatedText'])
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name=google_translate("Usage:", language)['translatedText'], value="`>rate <Anger | Fear | Joy | Sadness>`")
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name=google_translate("Example", language)['translatedText'], value=google_translate("If I'm feeling happy, I would `>rate Joy` 😊", language)['translatedText'])
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/845318994666848261/845399136249053205/logo_guy.png")
    return embed

def negative_ambiguous(language):
    embed = discord.Embed(title=google_translate("It looks like you might be feeling a little down.", language)['translatedText'], description=google_translate("We all have our ups and downs! Remember, sometimes, it's okay to not feel okay. Feel free to send me another message and I'll try to figure out how you are feeling!", language)['translatedText'], timestamp=datetime.datetime.utcnow(), color=discord.Color.from_rgb(153, 50, 204))
    embed.add_field(name=">rate", value=google_translate("You can also let me know how you are feeling with `>rate`!", language)['translatedText'])
    embed.add_field(name='\u200b', value='\u200b')
    embed.add_field(name=google_translate("Usage:", language)['translatedText'], value="`>rate <Anger | Fear | Joy | Sadness>`")
    embed.add_field(name='\u200b', value='\u200b')
...
async def rate(ctx,*,message=None):
    ...
    if message:
        ...
        if rating in ["anger", "fear", 'joy', 'sadness']:
            ...
            embed = discord.Embed(title="Oops!", description="Sorry! I couldn't quite pinpoint how you are feeling. Feel free to send me another message and I'll try to figure out how you are feeling!", timestamp=datetime.datetime.utcnow(), color=discord.Color.from_rgb(61, 72, 73))
